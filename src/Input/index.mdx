---
name: Input
menu: Data Entry
route: /data-entry/input
---

import { PropsTable, Playground } from 'docz';
import { MdAccountCircle } from 'react-icons/md';

import Button from '../Button';
import Textarea from './Textarea';
import Label from '../Form/Label';
import FormField from '../Form/FormField';
import Hint from '../Form/Hint';
import Icon from '../Icon';

import Input from './';

# Input

A basic widget for getting the user input is a text field. Keyboard and mouse can be used for providing or changing data.

## When To Use

- A user input in a form field is needed.
- A search input is required.

## Examples

```js
import { Input, Textarea, FormField, Hint } from '@yoctol/ui';
```

### Input

#### Basic

<Playground>
  <Input placeholder="placeholder" />
</Playground>

#### AutoSelect

<Playground>
  <Input
    autoSelect
    defaultValue="Exmaple default value"
    placeholder="placeholder"
  />
</Playground>

#### With FormField & Label

<Playground>
  <FormField>
    <Label htmlFor="demo">With Default:</Label>
    <Input id="demo" placeholder="placeholder" />
  </FormField>
</Playground>

#### With status

<Playground>
  <FormField>
    <Label htmlFor="default">With none:</Label>
    <Input id="none" placeholder="none status" />
  </FormField>
  <FormField>
    <Label htmlFor="default">With default:</Label>
    <Input id="default" placeholder="default status" />
    <Hint>default status</Hint>
  </FormField>
  <FormField success>
    <Label htmlFor="success">With Success:</Label>
    <Input id="success" placeholder="has success" />
    <Hint>has success</Hint>
  </FormField>
  <FormField warning>
    <Label htmlFor="warning">With Warning:</Label>
    <Input id="warning" placeholder="has warning" />
    <Hint>has warning</Hint>
  </FormField>
  <FormField error>
    <Label htmlFor="error">With Error:</Label>
    <Input id="error" placeholder="has error" />
    <Hint>has error</Hint>
  </FormField>
</Playground>

#### With disabled

<Playground>
  <Label htmlFor="demo">With Disabled:</Label>
  <Input id="demo" placeholder="placeholder" disabled />
</Playground>

#### With size

<Playground>
  <FormField>
    <Label htmlFor="demo_sm">
      With sm:
    </Label>
    <Input id="demo_sm" size="sm" placeholder="placeholder" />
  </FormField>
  <FormField>
    <Label htmlFor="demo_md">
      With md:
    </Label>
    <Input id="demo_md" size="md" placeholder="placeholder" />
  </FormField>
  <FormField>
    <Label htmlFor="demo_lg">
      With lg:
    </Label>
    <Input id="demo_lg" size="lg" placeholder="placeholder" />
  </FormField>
</Playground>

### Prefix an Suffix

<Playground>
  <Input prefix="Prefix" defaultValue="" />
  <Input mt="2" suffix="Suffix" defaultValue="" />
  <Input mt="2" suffix={<Button type="success">Send It</Button>} defaultValue="" />
  <Input mt="2" prefix="https://" suffix=".com" defaultValue="yoctol" />
  <Input
    mt="2"
    prefix={
      <>
        <Icon type={MdAccountCircle} size="20" mr="1"/>
        username:
      </>
    }
    defaultValue="yoctol"
  />
</Playground>

### Textarea

#### Basic

<Playground>
  <FormField>
    <Label htmlFor="demo">Basic Textarea</Label>
    <Textarea id="demo" placeholder="placeholder"/>
  </FormField>
</Playground>

#### With minRows & maxRows

<Playground>
  <FormField>
    <Label htmlFor="demo">With minRows & maxRows:</Label>
    <Textarea id="demo" placeholder="placeholder" minRows={3} maxRows={6} />
  </FormField>
</Playground>

## API

### Input

<PropsTable of={Input} />

### Textarea

<PropsTable of={Textarea} />
