...
import { Button, Box, Checkbox, FormField } from 'tailor-ui';
...
<Playground>
  <FormField required label="Checked Required" validator={boolean().oneOf([true]).required()}>
...
<Playground>
  <FormField
    ...
    validator={value => value && value.length !== 4 ? 'Should checked all check box' : null}
  >
    <Checkbox.Group
      defaultValue={['check_1']}
      options={[
